...
#ifndef xCPPCOUNTER_H_
#define xCPPCOUNTER_H_
 ...
 */
class xCounter : public SensorBase, public CounterBase
{
public:
	typedef enum {kTwoPulse=0, kSemiperiod=1, kPulseLength=2, kExternalDirection=3} Mode;

	xCounter();
	explicit xCounter(UINT32 channel);
	xCounter(UINT8 moduleNumber, UINT32 channel);
	explicit xCounter(DigitalSource *source);
	explicit xCounter(DigitalSource &source);
	explicit xCounter(AnalogTrigger *trigger);
	explicit xCounter(AnalogTrigger &trigger);
	xCounter(EncodingType encodingType, DigitalSource *upSource, DigitalSource *downSource, bool inverted);
	virtual ~xCounter();
