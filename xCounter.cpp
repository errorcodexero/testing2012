...
//	void SetAverageSize(int count);
//	int GetAverageSize();
//
// We had to do this by copy/paste rather than subclassing because WPILib makes
// all internal variables and functions 'private' rather than 'protected'.
// Aargh!
 ...
 */
xCounter::xCounter(UINT8 moduleNumber, UINT32 channel) :
	m_upSource(NULL),
	m_downSource(NULL),
	m_counter(NULL),
	m_encodingType(k1X)
 ...
 */
void xCounter::SetUpSourceEdge(bool risingEdge, bool fallingEdge)
{
	if (StatusIsFatal()) return;
	if (m_upSource == NULL)
	{
 ...
 */
void xCounter::SetDownSource(UINT32 channel)
{
	if (StatusIsFatal()) return;
 ...
 */
void xCounter::SetDownSourceEdge(bool risingEdge, bool fallingEdge)
{
	if (StatusIsFatal()) return;
 ...
 */
void xCounter::SetUpDownCounterMode()
{
	if (StatusIsFatal()) return;
	tRioStatusCode localStatus = NiFpga_Status_Success;
 ...
 */
void xCounter::SetExternalDirectionMode()
{
	if (StatusIsFatal()) return;
	tRioStatusCode localStatus = NiFpga_Status_Success;
 ...
 */
void xCounter::SetSemiPeriodMode(bool highSemiPeriod)
{
	if (StatusIsFatal()) return;
	tRioStatusCode localStatus = NiFpga_Status_Success;
 ...
 */
void xCounter::SetPulseLengthMode(float threshold)
{
	if (StatusIsFatal()) return;
	tRioStatusCode localStatus = NiFpga_Status_Success;
 ...
 * seconds.
 */
void xCounter::SetMaxPeriod(double maxPeriod)
{
	if (StatusIsFatal()) return;
	tRioStatusCode localStatus = NiFpga_Status_Success;
 ...
 */
void xCounter::SetUpdateWhenEmpty(bool enabled)
{
	if (StatusIsFatal()) return;
	tRioStatusCode localStatus = NiFpga_Status_Success;
...
/**
 * Set/query the number of samples for rate averaging.
 */
void xCounter::SetAverageSize(int size)
{
	if (StatusIsFatal()) return;
	tRioStatusCode localStatus = NiFpga_Status_Success;

	if (m_encodingType == k2X) {
	    size <<= 1;
	}
	...
	m_counter->writeTimerConfig_AverageSize(size, &localStatus);
	wpi_setError(localStatus);
}

int xCounter::GetAverageSize()
{
	if (StatusIsFatal()) return 1;
	tRioStatusCode localStatus = NiFpga_Status_Success;
	int size = m_counter->readTimerConfig_AverageSize(&localStatus);
	wpi_setError(localStatus);
	if (m_encodingType == k2X) {
	    size >>= 1;
	}
